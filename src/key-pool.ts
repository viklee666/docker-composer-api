 ...
 * 额度不足或 key 失效时由 KeyRotatingRunner 调用 disable 后换下一个。
 * 被禁用的 key 仅支持人工在管理后台重新启用。
 */
export class CursorKeyPool {
  ...
  async disable(id: string, kind: "quota" | "auth" | "manual", detail?: string): Promise<boolean> {
    const reason = kind === "quota" ? "额度不足" : kind === "auth" ? "key 无效" : "手动禁用";
    return this.store.updateCursorKey(id, {
      status: "disabled",
...
/**
 * 判断上游错误对 key 轮换的含义。
 * - quota：额度/配额耗尽（402、usage limit、quota、credit 等）→ 禁用该 key 并换下一个。
 * - auth：key 无效/被吊销（401、invalid api key 等）→ 禁用该 key 并换下一个。
 * - transient：上游以"无详情 error"收场（网关自抛的 upstream_run_failed）。原因不确定
 *   （额度耗尽、上游临时容量不足、auth 失败或模型不支持都可能，且 Cursor SDK 不在结果里给详情）。
 *   视为该 key 本次软失败 → 换下一个 key 重试，但不永久禁用，避免把临时故障误判成永久而误伤好 key。
