...
- **默认同会话持久本地 Agent**：runtime 仍是 local `Agent.create` + 多次 `send`，不是 `Agent.prompt()`（那是 create→一轮→dispose），也不是 Cloud Agents REST。`settingSources: []`。`customTools` 在 create/resume 时注册，后续 `send` 省略（一旦传入含 `{}` 会整表替换该次 Run）。有 identity 就复用同一 Agent（含显式头/body 与 CPA 短前缀）；认不出则 stateless fallback（本请求 create + 全文）。后续 `send` 只发本轮增量；客户端 `tool_result` 会 resolve 同一条 Run 上挂起的 `execute()`。`CURSOR_SDK_DISABLE_SESSION_RESUME=true` 是 kill switch：重启后回到 create + 全文 prompt + 假成功 + cancel + dispose。`CURSOR_SDK_SESSION_MODE=stateless` 在 kill switch 关闭时仍可显式关掉 durable Hub。改 mode 必须改 env 并重启，没有热切换。
