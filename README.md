...
- 默认同会话复用一个本地 Cursor Agent（`CURSOR_SDK_SESSION_MODE` 默认 `durable`；kill switch `CURSOR_SDK_DISABLE_SESSION_RESUME` 默认关闭）。可识别的会话（`x-session-affinity` / `x-opencode-session-id` / `anthropic-session-id`，或由 system + 首条 user 算出的 `conversationSeed`）会复用同一 agent，后续 `send` 只发增量；认不出则走 stateless。设 `CURSOR_SDK_DISABLE_SESSION_RESUME=true` 后**重启**，立即回到每请求 create + 全文 prompt + 假成功 + cancel + dispose。无论哪种模式都注入共享有界内存 agent store（闲置 TTL + LRU），避免 SDK 默认每 agent 一份 SQLite 导致句柄泄漏。改会话模式只能改 env 后重启，没有运行中热切换。
...
- **金额按 agent 记增量，摊不回具体某一次 run**：`agent.getUsage()` 给的是整个 agent 的累计值。durable 默认下一个 agent 会服务多个请求，网关按 agentId 记住上次记过的累计值（落库，重启也不丢），只把新增的那部分写进当前这条日志；基线推进与金额写入在同一事务里，写入失败不会吞掉基线。若累计值相对基线没有新增，网关会继续有限轮询而不是把旧值当成本次完成；即使观察到新增，也无法从累计接口证明它一定属于当前 run。因此同一 agent 上并发结束的两个请求可能一条拿到全部增量、另一条拿到 0——**单行金额是个近似，按 key / 按时间段的合计才是准的**。kill switch 打开时每个请求都是全新 agent，不落这笔基线。
...
- **默认同会话持久本地 Agent**：runtime 仍是 local `Agent.create` + 多次 `send`，不是 `Agent.prompt()`（那是 create→一轮→dispose），也不是 Cloud Agents REST。`settingSources: []`。`customTools` 在 create/resume 时注册，后续 `send` 省略（一旦传入含 `{}` 会整表替换该次 Run）。可识别的会话复用同一个本地 Agent：请求头 `x-session-affinity` / `x-opencode-session-id` / `anthropic-session-id`，或 `/v1/responses` 的 `previous_response_id` 链。Chat Completions / Anthropic Messages **不会**只用 system+首条 user 的哈希去复用 Agent（那会把互不相干的外部请求挤进同一把锁，表现为后台测试成功、外部 499）。后续 `send` 只发本轮增量；客户端 `tool_result` 会 resolve 同一条 Run 上挂起的 `execute()`。认不出会话则 stateless fallback（本请求 create + 全文）。`CURSOR_SDK_DISABLE_SESSION_RESUME=true` 是 kill switch：重启后回到 create + 全文 prompt + 假成功 + cancel + dispose。`CURSOR_SDK_SESSION_MODE=stateless` 在 kill switch 关闭时仍可显式关掉 durable Hub。改 mode 必须改 env 并重启，没有热切换。
- **缓存预期**：同会话后续轮次的 `cacheRead` 应明显高于首轮；不要单独用 `cacheRead / (cacheRead + cacheWrite)` 当下结论（首轮大量 write 是正常的）。更稳的口径是 `cacheRead / (input + cacheRead + cacheWrite)`。请求日志已存这些字段。
- **TTL**：挂起工具 `CURSOR_SDK_TOOL_HOLD_TTL_MS`（默认 15 分钟）；空闲 agent `CURSOR_SDK_SESSION_IDLE_TTL_MS`（默认 60 分钟）；同时存活会话上限 `CURSOR_SDK_MAX_LIVE_SESSIONS`（默认 256）。`REQUEST_TIMEOUT_MS` 只管这一条 HTTP。
